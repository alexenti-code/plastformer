...
def state_lines(model_path=SAFETY, limit=None):
    ...
    Это ПРЕДВАРИТЕЛЬНАЯ подача — названа ограничением, за подачу Φ-состояния
    не выдаётся. Настоящая подача — вектор записи, снятый с ядра и возвращённый
    в восприятие до внимания (узел F плана docs/BUILD-PLAN.ru.md).

    Исправлено 12.09.2026: здесь стояло «подавать надо проекцию вектора на
    основу полосы». Это была моя ошибка. Проекция нужна ЧУЖИМ векторам; наш
    снят с того же ядра и того же слоя — он уже в своём пространстве.
    """
