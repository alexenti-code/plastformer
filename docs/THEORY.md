...
4. **The write command arrives as an act in the model's output stream; the physics executes it.**
...
Descriptive note: temperature describes WHAT distribution text is sampled from without selecting content; forgetting_tempo is described in the same class. Enforceable boundaries for these dials (enumerated set, pre-registration, freezing) live in CONSTITUTION C3.
...
Continuity of an instance is described as following from preservation of Φ: the instance does not begin again with each session. Duplicating Φ is described plainly: the duplicate is a new instance that shares history with the original up to the moment of duplication and diverges afterwards. Dormancy is zero lived time, and waking after dormancy is a recorded event (see §4). (Copy = duplication, continuity = the Φ line: CONSTITUTION O-3/O-7.)
...
## 8. Honest boundary: what is built and what is not (state of 2026-09-12)

The weight file, the engine and the E1 protocol named in this section are built outside this repository and are not part of it.
...
- **BUILT (2026-09-12): the single weight file A = (K, Φ)** — 8.20 GB, with the Instruction embedded into K by the one pass. The Φ section is read and written at run time by the engine: it holds the eight functions of the Φ physics, and its return loop executes the model's acts and feeds back the `<<ENV>>` confirmation.
...
- There are **no results** in this document — E1 is pre-registered (see the E1 protocol v1.4 and preprint §7).
