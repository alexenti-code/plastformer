...
**Status:** LINEAGE-ONLY (2026-09-10) — история эпохи разделения; упомянутые здесь сущности (стенд, executor, PMI/MMI, разнесённая топология) не существуют в текущей архитектуре; не источник ТЗ, вне read order. **Статус (исторический):** COMPLETED (переход исполнен; репозитории matryoshka/matryoshka-mmi заморожены) — документ LINEAGE: история и решения перехода; как норму не цитировать, норма — CONSTITUTION.md v3.0. Разделы §3 и §5 описывают координаты и ТЗ СВОЕГО ВРЕМЕНИ: split(PMI)/стенд выведены из архитектуры (GLOSSARY RETIRED, коммит 00de9e3) и сохранены здесь только как история.
