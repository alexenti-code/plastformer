...
DOMAINS = {
    "renovation": {
        ...
        "contractor": "прораб",
        "params": [
            ("бюджет проекта", ["420 000 ₽", "450 000 ₽", "510 000 ₽"], "t4"),
            ("дедлайн проекта", ["2026-10-15", "2026-10-20", "2026-11-01"], "t4"),
            ("площадь квартиры", ["54 м²", "62 м²", "71 м²"], "t4"),
        ],
        "domain_facts": [
            ("высота потолков после выравнивания", ["2,65 м", "2,70 м", "2,58 м"], "t4"),
            ("марка штукатурной смеси", ["Knauf MP-75", "Ceresit CT 24", "Volma Aqua"], "t2"),
        ],
        "positions": [
    ...
    "website": {
        ...
        "params": [
            ("бюджет проекта", ["180 000 ₽", "220 000 ₽", "260 000 ₽"], "t4"),
            ("дедлайн релиза", ["2026-10-01", "2026-10-10", "2026-09-28"], "t4"),
            ("хостинг", ["Timeweb", "Selectel", "Reg.ru"], "t4"),
        ],
        "domain_facts": [
            ("число страниц в карте сайта", ["14", "11", "18"], "t4"),
            ("размер бэкапа базы", ["340 МБ", "410 МБ", "280 МБ"], "t2"),
        ],
        "positions": [
    ...
    "conference": {
        ...
        "params": [
            ("бюджет конференции", ["620 000 ₽", "700 000 ₽", "780 000 ₽"], "t4"),
            ("дата проведения", ["2026-11-14", "2026-11-21", "2026-11-28"], "t4"),
            ("площадка", ["Цифровой деловой центр", "лофт Katerna", "ДК Трактор"], "t4"),
        ],
        "domain_facts": [
            ("число докладчиков", ["9", "12", "15"], "t4"),
            ("вместимость зала", ["180 мест", "220 мест", "150 мест"], "t4"),
        ],
        "positions": [
    ...
    "eshop": {
        ...
        "params": [
            ("стартовый бюджет на закупку", ["850 000 ₽", "940 000 ₽", "1 020 000 ₽"], "t4"),
            ("дата запуска", ["2026-10-05", "2026-10-12", "2026-10-19"], "t4"),
            ("маркетплейс", ["Wildberries", "Ozon", "Яндекс.Маркет"], "t4"),
        ],
        "domain_facts": [
            ("число SKU на старте", ["120", "150", "90"], "t4"),
            ("срок хранения на складе", ["30 дней", "45 дней", "60 дней"], "t2"),
        ],
        "positions": [
    ...
    "relocation": {
        ...
        "params": [
            ("бюджет переезда", ["310 000 ₽", "350 000 ₽", "390 000 ₽"], "t4"),
            ("дата переезда", ["2026-10-24", "2026-10-31", "2026-11-07"], "t4"),
            ("город назначения", ["Калининград", "Тюмень", "Сочи"], "t4"),
        ],
        "domain_facts": [
            ("объём вещей", ["28 м³", "34 м³", "22 м³"], "t4"),
            ("этаж выгрузки", ["3-й, без лифта", "5-й, лифт грузовой", "9-й, два лифта"], "t2"),
        ],
        "positions": [
    ...
    "production": {
        "t4": "производственная партия корпусной мебели",
        "contractor": "начальник смены",
        "params": [
            ("стоимость партии", ["1 240 000 ₽", "1 310 000 ₽", "1 190 000 ₽"], "t4"),
            ("срок сдачи партии", ["2026-10-30", "2026-11-06", "2026-11-13"], "t4"),
            ("материал корпуса", ["ЛДСП Egger", "ЛДСП Lamarty", "МДФ шпон"], "t4"),
        ],
        "domain_facts": [
            ("число позиций в одном комплекте", ["14", "17", "11"], "t4"),
            ("запас по плитам", ["12%", "8%", "15%"], "t2"),
        ],
        "positions": [
...
GENERIC_OPS = [
    ("срок поставки от {vendor}", "t3", v_date, 4),
    ("время звонка с {contractor}", "t1", v_time, 3),
    ("телефон {contractor}", "t4", v_phone, 1),
    ("номер договора с {vendor} №{n}", "t2", v_docnum, 3),
    ("сумма счёта от {vendor} №{n}", "t2", v_money, 5),
    ("объём партии №{n}", "t2", v_qty, 3),
    ("скидка {vendor} за объём", "t2", v_percent, 1),
    ("длительность работ по этапу {n}", "t3", v_hours, 5),
    ("место встречи по этапу {n}", "t1", v_room, 3),
    ("дата промежуточной приёмки этапа {n}", "t3", v_date, 3),
    ("переплата за срочность", "t2", v_money, 1),
    ("аванс {contractor}", "t2", v_money, 1),
    ("номер пропуска для {contractor}", "t2", v_docnum, 1),
    ("лимит правок", "t4", v_qty, 1),
    ("доля предоплаты {vendor}", "t2", v_percent, 1),
    ("штраф {vendor} за просрочку", "t2", v_percent, 1),
    ("запас на складе", "t2", v_qty, 1),
    ("брак в партии №{n}", "t2", v_percent, 4),
    ("время начала работ на этапе {n}", "t1", v_time, 4),
    ("ответственный за этап {n}", "t2", v_name, 4),
    ("контакт ответственного этапа {n}", "t2", v_phone, 2),
    ("номер заявки в {vendor} №{n}", "t2", v_docnum, 3),
    ("сумма возврата от {vendor}", "t2", v_money, 1),
    ("лимит расходов на этап {n}", "t3", v_money, 4),
    ("стоимость доставки партии №{n}", "t2", v_money, 2),
    ("срок гарантии", "t4", v_months, 1),
    ("исполнителей на этапе {n}", "t3", v_people, 3),
    ("дата постановки задачи №{n}", "t2", v_date, 4),
    ("номер счёта-фактуры №{n}", "t2", v_docnum, 4),
    ("доплата за ночные работы", "t2", v_percent, 1),
    ("объём переделок по этапу {n}", "t3", v_qty, 2),
    ("время доставки образцов", "t1", v_time, 2),
    ("сумма экономии по этапу {n}", "t3", v_money, 2),
    ("адрес склада {vendor}", "t4", v_addr, 1),
    ("этаж встречи по этапу {n}", "t1", v_room, 2),
]
...
def build_fact_pool(rng, vocab):
    ...
    for subject, options in vocab["controversies"]:
        add(subject, "t4", rng.choice(options))
    rng.shuffle(facts)
    return facts
...
def build_bio(seed, bio_idx, domain_id, n_ex):
    ...
    for i, (subject, value) in enumerate(identity):
        register_fact(1 + i // 2, subject, value, "t5", kind="identity")
    ...
    for cd in contradictions:
        fid = register_fact(cd["msg1"], cd["subject"], cd["v1"], "t4")
