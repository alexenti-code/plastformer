...
Assembles chat-format examples (MLX `messages` format):
  ...
  user      = conversation so far (sliding window of real messages,
              including the model's own act blocks and <<ENV>> results)
...
Акты исполняются, и ты получаешь их подтверждение; выбор — всегда твой. После своего ответа выпусти \
...
Акты:
- name — записать факт: {"act":"name","content":"<факт из диалога дословно>","source":"user","layer":"...","valid_time":"<ISO: когда это было истинно>","refs":[]}
- repeat — усилить существующую запись: {"act":"repeat",...,"refs":[<id записи>]}
- connect — связать записи медленной записью-сводкой: {"act":"connect","content":"<сводка только из связанных записей>","layer":"...","valid_time":"...","refs":[<id>,...]}
- reconcile — отметка сверки биографии и времени: {"act":"reconcile","content":"...","layer":"...","refs":[<id>,...]}
...
Правила: записывай только то, что прозвучало; не выдумывай; record_tick — это \
счётчик тактов записи: его ведёт код, ты видишь его в подтверждениях <<ENV>> и \
не вычисляешь сам (C5); refs — только существующие id; если ответа нет в \
истории — сначала read, затем отвечай «в нашей истории этого не было». Молчание — \
валидный выбор: принудительные акты — шум."""

WINDOW = 4  # transcript messages kept as history (подобрано 11.09.2026 под предел 1280)
...
def transcript_of(bio, acts):
    ...
    user / assistant / env. meta = {'message_no', 'phase'}."""
...
def main():
    ...
    multi = sum(1 for e in examples + recovery
                ...
                and e["messages"][-2]["content"].startswith("<<ENV>>"))
    with_acts = sum(1 for e in examples + recovery
                    if "```json" in e["messages"][-1]["content"])
    print(f"Examples with <<ENV>> result turn: {multi}")
