...
- **Phase B** — self-improvement mode (the master switch `self_improvement`, owner directive 07.09, clarified 07.09): when ON, the model works with its memory **by itself, at any moment** — not only tied to ticks/inference runs. Two scans (owner): **`scan`** — the model's own act of reading its physically recorded layered memory (read-class, can enter the act grammar now; raw amplitudes/ticks/layers, no scores, no writes, no ticks per C5, no trace — remembering only via explicit act; budget ≤ 32/run); and the **environment scan** — improving the memory mechanisms themselves (τ layers, thresholds, budgets), done through `calibrate` (≤ 10/run) since only explicit acts change the environment (C4). The environment only validates physics bounds (reject-not-clamp, O-10); accepted changes apply to the NEXT episode (C3). The switch itself is not core-controllable. Oracle-derived metrics stay harness-side (C7). The offline phase does NOT advance lived ticks (protocol.md §6). Full standalone operation (core time without a user at all, background tick, THEORY §4) remains a deferred milestone — requires non-inference compute windows; separate session.
