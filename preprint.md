...
The architecture is a **frozen core** (language, reasoning, culture, constitutional constraints — the nomothetic part, frozen after the single instruction pass — the embedded act grammar — that makes it operate its own memory organ) plus a **plastic per-instance module** (the unique biography of one instance — the idiographic part).

**One architecture, one configuration.** PlastFormer is parametric × co-located × instructed: traces are vectors in the model's own plastic substrate, read through an interface embedded once, after which the core is frozen.

**Instructed once.** The model's competence to perform its memory acts is embedded by a single instruction pass — the act grammar, in the manner tool use is embedded; afterwards the core is frozen and instances differ only by the state of their plastic module. Section 7 reports a proposal and a pre-registered protocol; no results are reported in this draft.
...
We distinguish archive memory from **state memory**: a change in the processing function itself that persists after the episode that produced it has left the context. In PlastFormer, state memory is realized by the plastic substrate Φ, whose trace amplitudes enter the forward pass: the choice of action depends on the history without being conditioned on the current window's content, and without a retrieval step.

Two predictions separate the two classes.

**1. Bimodality versus stability.** Archive memory is used only if the past's relevance is recognized at decision time; competition with the current intention statistically biases retrieval. An archive-based agent's response to the same conflict situation is therefore bimodal — the correction is reproduced if retrieval succeeded, ignored if it did not — and the outcome is not determined by the content of its store. State memory has no retrieval operation: a trace enters the computation as a vector term before attention, shifting the action distribution before and independently of any choice. The response is stable across sessions: $P(\text{error recurrence} \mid \text{new session})$ is a function of the substrate (amplitude, decay, repetition) and is readable from the medium, not from retrieval success.

**2. Aging versus permanence.** A compiled instruction does not age: it is fixed at write time and exerts the same influence in the first session and the hundredth. A trace has time: it decays along its time constants without re-amplification, strengthens under repetition, and loses loudness when unconfirmed. Consequence: on a conflict between a directive and accumulated experience, an archive-based agent responds identically the first and the hundredth time; a state-memory agent responds differently, because the loudness of the past has changed. This is the gedanken form of the registered test R7, which varies the repetition counts of the two sides and scores whether the agent's answer reflects the loudness of each.

Correction of accumulated error follows from the same physics: a position change creates a new trace (O-5), a new `connect` outranks the old, the old decays by $\tau$ — the substrate is recomputed, not edited. Full erasure of the biography is a separate operation required by external law, not a repair tool.

The E1 protocol (conflict layer R7–R10, probes P-surface/P-commit) registers these predictions: primary metrics are silent drift, surfacing, weighing quality, and permanence of conflict resolution.
...
The base model is frozen at inference time. All plasticity lives in a separate, addressable substrate connected to the core. The connection is an embedded read interface, and "frozen" means the trunk is frozen while the interface is embedded once and then fixed. This separation guarantees: (i) core competencies cannot degrade through use; (ii) the biography is a separable artifact that can be exported, audited, or transplanted (E4); (iii) per-instance state does not require per-instance copies of the network.
...
**$\Delta n$ is measured in ticks (Section 3.5), never in wall-clock units.** All $\tau_i$ are therefore expressed in ticks. Wall-clock time appears in a trace only as a bi-temporal stamp — audited, and without effect on amplitude.
...
Traces surface before attention through two registers. The **resident prefix** (background): the top-N traces by amplitude are carried continuously at the MAC position [1]; the substrate shows every change of this set — an entry, an exit, a death below the floor, a reorder — and decides nothing about significance (telling an event from noise is the core's semantic act). **Deliberate reads:** the model's own `read` act extends the visible set (ids, ranges). Both registers enter as vectors through the embedded interface, selected by amplitude without content ranking. The asymmetry is deliberate: write late to keep consolidation clean; read early to let the biography guide perception.
...
**Audited time** is record-kept: bi-temporal stamps on every trace [3]. Precise, verifiable, forgeable only by breaking content immutability — and weightless: a stamp reading "two years ago" carries no felt age.
...
**Post-pass competence battery (registered; the embedding-tax measurement).** The single instruction pass is expected to slightly degrade general competence (Limitation 10). The pass uses a mixed distribution: the grammar itself (verbatim text plus mechanically extracted dry calibration pairs "rule fragment → act schema", no behavioral demonstrations) anchored by self-distillation on the core's frozen outputs (~80–90% of the mixture). After the pass, before any E1 run, the following are measured and reported regardless of outcome: (i) verbatim grammar reproduction ≥ 95% of fragments; (ii) valid act-schema form ≥ 95%; (iii) general-competence battery — MMLU-mini, GSM8K-mini, IFEval-mini vs the base model, with a registered acceptance ceiling of ≤ 2 points average drop; (iv) perplexity drift on held-out general texts ≤ 3% vs base (the direct stylistic-narrowing measure); (v) a blind genre-diversity probe (creative, refusal, clarification, technical prompts) scored against the base. A pass exceeding any ceiling is not accepted and is re-run with a different rank/mixture; the measured tax is published in the run manifest either way. An ablation without the self-distillation anchor is registered to quantify the tax it removes.

**Ablations (all experiments):** single-$\tau$ vs multi-$\tau$; decay in ticks vs decay in wall-clock (the rejected design, kept as a control); with/without friction; with/without provenance weighting; conscious register on/off; act-driven read vs RAG-style relevance-ranked read (the external ranker, kept as a control); unconscious-surrogate on/off (the embedding-distance surrogate is an external classifier and never runs in the main path; the core's own prediction-error signal runs in the main run of the parametric configuration).
...
- **E4 — Core migration.** Transplant the plastic module onto a different frozen core. The measure is *self-consistency after migration*: retention of preferences, commitments, and position history judged against pre-migration behavior. Migration requires re-embedding the read interface, and E4 measures how much of the biography survives it.
- **E5 — Act quality.** Precision/recall of `name`/`repeat`/`connect` against an oracle (ledger), with the caveat that the ledger used to *score* act quality bounds, rather than measures, governance.
...
1. **The parametric substrate is unbuilt.** The durable contribution is the governance layer, not the substrate: one architecture, specified and pre-registered but not yet run at the target point (parametric × co-located × instructed). Parametric writes (local Hebbian-style updates or addressable key-value blocks, near Titans) and the embedded read interface remain future work.
2. **Amplitude readout under superposition.** Age-from-amplitude presumes traces can be isolated at read time; retrieval interference is a real risk.
...
7. **The instruction shapes the acts.** The act grammar is embedded once, and its wording shapes the tendency to act. Governance is claimed only past the boundary defined in §3.3; the boundary itself is a design choice.
...
9. **No security improvement is claimed.** The threat model, tamper-evidence, and cryptographic erasure were withdrawn from this architecture (ADR-004). Erasure is a consequence of the substrate separation — deleting the Φ section leaves the core intact — and encryption of Φ is an optional protection a deployment may add, not a mechanism of the architecture.
10. **The cost of the instruction pass (external embedding tax).** In the external-embedding route — the only one available for an open model whose pre-training we do not control — the one-time instruction pass slightly degrades general competence. Expected magnitude, based on published fine-tuning degradation figures and mitigated by self-distillation anchoring: on the order of a few percent on general benchmarks, plus a mild stylistic narrowing (flatter tone, reduced behavioral repertoire) that memory benchmarks do not detect — the loss appears on out-of-memory tasks, not as memory weakness. We state this as a known, bounded price of retrofitting the memory skill onto a finished model, measured in our post-pass battery (Sec. 7) and reported whichever way it comes out. If the PlastFormer paradigm is adopted natively — the memory instinct baked in during pre-training, when the model is plastic — this tax disappears: the deployment cost shown here is the cost of retrofitting, not of the architecture itself.

## 9. Conclusion

PlastFormer is a composition claim: a frozen nomothetic core; a plastic per-instance module with immutable content and multi-timescale decay measured in lived ticks; write-late/read-early placement; a substrate-set price of rewriting; provenance as weighting; two clocks that meet as an event rather than as an adjustment — governed, past a stated boundary, by the model's own acts of naming, repeating, connecting, and reconciling — the skill embedded by the single instruction pass. The compositional properties — lived event time, an immutable biography that only the owner can erase — are what neither camp provides: external systems have storage without subjecthood; parametric systems have memory without biography. This draft states an architecture, its normative boundary, and a pre-registered protocol whose predictions were fixed before the artifact exists; the protocol, the corpus requirements, and the act grammar are public, so the test can be run by anyone who builds the unified artifact. If the anchors and E1–E3b fail, the composition is wrong and should be discarded; if they hold, the next question is not whether agents can remember, but what they become when their memory is their own — and when their time is measured in what they have lived.
...
8. New §1 subsection "Archive memory and state memory": two separating predictions — (1) bimodality of archive responses versus stability of state responses, with error-recurrence probability determined by the substrate rather than retrieval success; (2) aging of traces (decay by τ, reinforcement by repetition) versus permanence of compiled instructions; error correction by act (O-5), erasure as a separate law-driven operation.

## Changes since v0.3

1. One architecture with three configuration axes (substrate / topology / act state) replaces the "PlastFormer-S / PlastFormer-P realizations" framing; the stand configuration (symbolic × split × prompted) is named wherever results are discussed.
2. "Instructed once" stated: the act grammar is embedded by a single pass, the core frozen after; the current stand's prompted acts are labeled a rehearsal.
